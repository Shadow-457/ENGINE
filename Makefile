...
fat32.img:
	...
	@# Copy everything inside home/ into the FAT32 image root
	@for f in home/*; do \
	    [ -f "$$f" ] || continue; \
	    MTOOLS_SKIP_CHECK=1 mcopy -i $@ $$f ::/`basename $$f` && echo "  Added `basename $$f` to FAT32"; \
	done

# -- Combined bootable disk image ----------------------------------
systrix.img: boot.bin kernel.bin
	rm -f fat32.img
	dd if=/dev/zero of=fat32.img bs=1M count=64 status=none
	mkfs.fat -F 32 -n SYSTRIXOS fat32.img
	echo "Systrix OS v0.1 (C kernel)" > /tmp/README.TXT
	MTOOLS_SKIP_CHECK=1 mcopy -i fat32.img /tmp/README.TXT ::/README.TXT
	@# Copy everything inside home/ into the FAT32 image root
	@for f in home/*; do \
	    [ -f "$$f" ] || continue; \
	    MTOOLS_SKIP_CHECK=1 mcopy -i fat32.img $$f ::/`basename $$f` && echo "  Added `basename $$f` to FAT32"; \
	done
